...
async function testAddToCartAsGuest(): Promise<void> {
  ...
  const first = items[0];
  if ((first.quantity as number) !== 2)
    throw new Error(`expected qty 2, got ${first.quantity}`);
  if (typeof body?.subtotal !== 'number') throw new Error('missing subtotal');
  if (typeof body?.orderTotal !== 'number')
...
async function testGetCartReturnsItems(): Promise<void> {
  ...
  const { status, body } = await request('GET', '/cart', {
    cookie: guestCookie,
  });
...
async function testUpdateCartQuantity(): Promise<void> {
  ...
  if (status !== 200) throw new Error(`expected 200, got ${status}`);
  const items = (body?.cart as Record<string, unknown>).items as Record<
    string,
    unknown
  >[];
  if ((items[0].quantity as number) !== 5)
    throw new Error(`expected qty 5, got ${items[0].quantity}`);
}
...
async function testClearCart(): Promise<void> {
  ...
  if ((body?.orderTotal as number) !== 0)
    throw new Error('expected orderTotal 0');
}
