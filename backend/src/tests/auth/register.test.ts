...
async function testRegisterHappyPath(): Promise<void> {
  ...
  if (status !== 201)
    throw new Error(`register happy path: expected 201, got ${status}`);
  ...
  if (!(body?.user as Record<string, unknown>)?._id)
    throw new Error('register happy path: missing _id');
...
async function testRegisterDuplicateEmail(): Promise<void> {
  ...
  if (status !== 400)
    throw new Error(`duplicate email: expected 400, got ${status}`);
}
...
async function testRegisterMissingName(): Promise<void> {
  ...
  if (status !== 400)
    throw new Error(`missing name: expected 400, got ${status}`);
}
...
async function testRegisterShortPassword(): Promise<void> {
  ...
  if (status !== 400)
    throw new Error(`short password: expected 400, got ${status}`);
}
