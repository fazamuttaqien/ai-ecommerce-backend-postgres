...
async function testLogoutAuthenticated(): Promise<void> {
  ...
  const { status, body, setCookie } = await request('POST', '/auth/logout', {
    cookie,
  });

  if (status !== 200)
    throw new Error(`authenticated: expected 200, got ${status}`);
  if (body?.message !== 'User logged out successfully')
    throw new Error(`authenticated: wrong message: ${body?.message}`);
  if (
    !setCookie.includes('instant_access_token=;') &&
    !setCookie.includes('instant_access_token=')
  )
...
async function testStatusAfterLogout(): Promise<void> {
  ...
  if (status !== 401)
    throw new Error(`status after logout: expected 401, got ${status}`);
}
