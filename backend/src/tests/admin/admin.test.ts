...
async function testAnalyticsAsAdmin(): Promise<void> {
  if (!adminCookie) {
    process.stderr.write(
      'skipped analytics admin test: no admin credentials\n',
    );
    return;
  }
  const { status, body } = await request('GET', '/admin/analytics', {
    cookie: adminCookie,
  });
  if (status !== 200) throw new Error(`expected 200, got ${status}`);
  if (body?.message !== 'Admin analytics retrieved successfully')
    throw new Error(`wrong message: ${body?.message}`);
  if (typeof body?.totalSales !== 'number')
    throw new Error('missing totalSales');
  if (typeof body?.totalOrders !== 'number')
    throw new Error('missing totalOrders');
  if (typeof body?.totalProducts !== 'number')
    throw new Error('missing totalProducts');
}
