...
async function testCreateOrderCOD(): Promise<void> {
  ...
  if (order.status !== 'placed')
    throw new Error(`expected status placed, got ${order.status}`);
...
async function testGetOrderById200(): Promise<void> {
  ...
  const { status, body } = await request('GET', `/orders/${orderId}`, {
    cookie: userCookie,
  });
  if (status !== 200) throw new Error(`expected 200, got ${status}`);
...
async function testGetOrderById404(): Promise<void> {
  if (!userCookie) throw new Error('no cookie');
  const { status } = await request('GET', '/orders/507f1f77bcf86cd799439011', {
    cookie: userCookie,
  });
  if (status !== 404) throw new Error(`expected 404, got ${status}`);
}
