...
async function setupUserAndOrder(): Promise<void> {
  ...
  if (reg.status !== 201)
    throw new Error(`setup: register failed, got ${reg.status}`);
  ...
  if (cartRes.status !== 200)
    throw new Error(`setup: add to cart failed, got ${cartRes.status}`);
  ...
  if (addrRes.status !== 201)
    throw new Error(`setup: create address failed, got ${addrRes.status}`);
  ...
  if (ordRes.status !== 201)
    throw new Error(`setup: create order failed, got ${ordRes.status}`);
  ...
  if (!items || items.length === 0)
    throw new Error('setup: order has no items');
...
async function testCreateReviewNotDelivered(): Promise<void> {
  ...
  if (!userCookie || !orderId || !orderItemId)
    throw new Error('no setup state');
  const { status } = await request('POST', '/reviews', {
    cookie: userCookie,
...
async function testGetReviewableReturns200(): Promise<void> {
  if (!userCookie) throw new Error('no cookie');
  const { status, body } = await request('GET', '/reviews/reviewable', {
    cookie: userCookie,
  });
...
async function testGetUserReviewsReturns200(): Promise<void> {
  if (!userCookie) throw new Error('no cookie');
  const { status, body } = await request('GET', '/reviews', {
    cookie: userCookie,
  });
  if (status !== 200) throw new Error(`expected 200, got ${status}`);
