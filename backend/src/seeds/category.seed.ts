...
const seedCategories = async () => {
  try {
    ...
    // Prisma's createMany doesn't run per-row hooks (Prisma has no
    // Mongoose-style middleware), so the slug that used to be derived in
    // `categorySchema.pre('validate')` is computed here instead.
